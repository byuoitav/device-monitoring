...
var (
	piHostname = os.Getenv("PI_HOSTNAME")
...
func init() {
	...
	split := strings.Split(piHostname, "-")
	if len(split) != 3 {
		log.L.Fatalf("invalid PI_HOSTNAME %v.", piHostname)
	}
...
// Run runs the job.
func (m *MonitoringJob) Run(ctx interface{}, eventWrite chan events2.Event) {
	log.L.Infof("Getting room status")

	resp, err := http.Get(fmt.Sprintf("http://localhost:8000/buildings/%v/rooms/%v", buildingID, room))
	if err != nil {
	...
	for _, dev := range status.Displays {
		if strings.Contains(dev.Name, "-") {
			event.TargetDevice = events2.GenerateBasicDeviceInfo(dev.Name)
		} else {
			event.TargetDevice = events2.GenerateBasicDeviceInfo(fmt.Sprintf("%v-%v-%v", buildingID, room, dev.Name))
		}
