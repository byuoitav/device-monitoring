...
export class AppComponent implements OnInit {
  ...
  rebootVias() {
    ...
    let endpoint =
      ":8000/buildings/" + split[0] + "/rooms/" + split[1] + "/configuration";

    this.api.localget(endpoint).subscribe(data => {
      ...
      for (let device of data["devices"]) {
        if (device.type == "via") {
          ...
          this.rebootViaMessages.push(
            "rebooting " + device.name + " @ " + device.address
          );

          this.api
            .localget(":8014/via/" + device.address + "/reboot")
            .subscribe(
              data => {
                this.rebootViaMessages.push(
                  "successfully rebooted " + device.name
                );
              },
              err => {
