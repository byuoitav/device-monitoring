...
func StartMonitoring(interval time.Duration, apiAddr string, building string, room string) {
...
func GetAndReportStatus(addr string) error {
	...
	if err != nil {
		errStr := fmt.Sprintf("There was a problem decoding the respone from the API", err.Error())
		log.Printf("[api-status]" + errStr)
		PublishError(errStr, eventinfrastructure.AUTOGENERATED)
		return err
	}
